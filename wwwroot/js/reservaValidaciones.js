...
function verificaCant() {
    //verfica que se seleccionen adultos
    let adul = document.getElementById("cantAdultos").value;
    let may = document.getElementById("cantMayores").value;
    console.log(adul);
    console.log(may);
    if ((+adul == 0) && (+may == 0)) {
        document.getElementById("errorCant").innerHTML = "Debe seleccionar al menos un adulto o adulto mayor";

        $("#cancelarReserva").prop('disabled', true);
    }
    else {
        $("#cancelarReserva").prop('disabled', false);
